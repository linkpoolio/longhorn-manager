...
func (s *TestSuite) TestReconcileInstanceState(c *C) {
	testCases := map[string]struct {
		...
		// Regression: volumes stuck attaching after an IM roll. The mocked
		// handler cannot surface the production GetInstance error, so this
		// case pins that reconcile proceeds through the create path (the
		// stale ref is healed, then cleared by the not-found sync);
