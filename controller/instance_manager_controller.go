...
func (imc *InstanceManagerController) isResponsibleForSetting(obj interface{}) bool {
	...
	return types.SettingName(setting.Name) == types.SettingNameKubernetesClusterAutoscalerEnabled ||
		...
		types.SettingName(setting.Name) == types.SettingNameDataEngineShallowCopyPipelineDepth ||
...
func (imc *InstanceManagerController) areDangerZoneSettingsSyncedToIMPod(im *longhorn.InstanceManager) (isSynced, isPodDeletedOrNotRunning, areInstancesRunningInPod bool, err error) {
	...
	for settingName := range types.GetDangerZoneSettings() {
		...
		case types.SettingNameDataEngineShallowCopyPipelineDepth:
			isSettingSynced, err = imc.isV2ReplicaTimeoutEnvSynced(im, pod, settingName, types.EnvV2ShallowCopyPipelineDepth)
		}
		if err != nil {
			return false, false, false, err
		}
...
func (imc *InstanceManagerController) getV2ReplicaTimeoutEnv(dataEngine longhorn.DataEngineType) ([]corev1.EnvVar, error) {
	pairs := []struct {
		...
		{types.EnvV2ShallowCopyPipelineDepth, types.SettingNameDataEngineShallowCopyPipelineDepth},
	}
