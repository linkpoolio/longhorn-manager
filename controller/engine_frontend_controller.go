...
func NewEngineFrontendController(
	...
	efc := &EngineFrontendController{
		baseController: newBaseControllerWithCappedBackoff("longhorn-engine-frontend", logger, time.Minute),
...
func (efc *EngineFrontendController) syncEngineFrontend(key string) (err error) {
	...
	defer func() {
		persistStatusObservedByErroredSync(!reflect.DeepEqual(existingEF.Status, ef.Status), func() error {
			_, updateErr := efc.ds.UpdateEngineFrontendStatus(ef)
			return updateErr
		}, &err, log, "engine frontend")
		if apierrors.IsConflict(errors.Cause(err)) {
